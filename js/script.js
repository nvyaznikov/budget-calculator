...
const totalSummAccumulationYear = document.querySelector(
  ".total_summ_accumulation_year span"
);
...
switchThema.addEventListener("click", () => {
  ...
  const timeCookie = switchThema.classList.contains("active")
    ? 3600 * 24 * 7
    : 0;
...
const inputIncomeCategory = document.querySelector(
  ".section_incomes .category_text_profits"
);
const inputIncomeAmount = document.querySelector(
  ".section_incomes .amount_text_profits"
);
const inputIncomeDate = document.querySelector(
  ".section_incomes .date_text_profits"
);
...
const inputSpendCategory = document.querySelector(
  ".all_spends .category_text_spends"
);
const inputSpendAmount = document.querySelector(
  ".all_spends .amount_text_spends"
);
...
const buttonAddRegularWaste = document.querySelector(
  ".button_add_regular_waste"
);

const selectRegularCategory = document.querySelector(
  ".regular_spends .regular_category"
);
const inputRegularAmount = document.querySelector(
  ".regular_spends .regular_amount"
);
const inputRegularDate = document.querySelector(
  ".regular_spends .regular_date"
);

let arrayIncomes = localStorage.getItem("incomes")
  ? JSON.parse(localStorage.getItem("incomes"))
  : []; // массив с доходами
let arrayCosts = localStorage.getItem("costs")
  ? JSON.parse(localStorage.getItem("costs"))
  : []; // массив с расходами
let arrayRegularCosts = localStorage.getItem("regularCosts")
  ? JSON.parse(localStorage.getItem("regularCosts"))
  : []; // массив с постоянными расходами
...
function deleteRegularCosts(button) {
  button.addEventListener("click", () => {
    const question = confirm(
      "Вы действительно хотите удалить регулярный доход?"
    );
    if (!question) return;
...
function randerRegularCosts() {
  ...
  const data = localStorage.getItem("regularCosts")
    ? JSON.parse(localStorage.getItem("regularCosts"))
    : [];
